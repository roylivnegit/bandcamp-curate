# crate-digger — working notes for Codex
...
feed of tracks you don't own yet. Full build plan: `~/.Codex/plans/i-want-to-create-purrfect-pascal.md`.
...
- **Frontend:** separate React app (`frontend/`, Vite + TypeScript), own origin, talks to the API
  over CORS with a JWT bearer token. **Read `frontend/AGENTS.md` before touching it** — the React
...
- **Fan-out is bounded by depth** (`crawl_frontier.depth`, seed=0; `crawl_max_depth` config,
  ...
  provider fetches, all-time, **global across every user and scan** — there is no per-user budget
  yet, see "Immediate next steps" below; enforced in `runner.run_until_empty` + the ARQ `crawl_next`
  chain). Ingest still happens at the boundary, only outward enqueue stops.
...
`~/.Codex/skills/parser-builder/`) documents the exact format offline (no credits to iterate):
...
## Conventions
- Commit per milestone; end messages with `Co-Authored-By: Codex Opus 4.8 <noreply@anthropic.com>`.
