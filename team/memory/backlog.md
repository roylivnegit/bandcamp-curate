...
- [x] **"Back to top" affordance for long feeds.** *(proposed by the hourly routine, 2026-09-02,
  ...
  way back to the filter bar/top once scrolled deep. Done in a separate PR from this same
  routine run — see the `components/ScrollTopButton.tsx` entry elsewhere in this file's history
  (git log) for the full writeup; summarizing here since this file's own history has two
  divergent copies of that entry from parallel branches in the same round: a `useState`
  initialized from `window.scrollY` plus a passive `scroll` listener toggles visibility past
  `SCROLL_TOP_THRESHOLD_PX` (600, in `config.ts`); on click it calls `window.scrollTo({top:0,
  behavior:'smooth'})` then refocuses the page heading via the existing `headingRef` pattern.
  4 standalone tests + 1 integration test in `feed.test.tsx`. PR: see git history.
